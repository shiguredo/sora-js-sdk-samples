...
const connectSendrecv = async () => {
  ...
  sendrecv.on("disconnect", (event) => {
    document.querySelector<HTMLVideoElement>("#local-video")!.srcObject =
      localStream;
  });
...
const connectScreenCapture = async () => {
  ...
  sendonly.on("removetrack", (event) => {
    ...
    if (target) {
      document
        .querySelector<HTMLVideoElement>(`#remote-video-${target.id}`)!
        .remove();
    }
  });
  ...
  // gDM で画面の映像を取得する
  screeCaptureStream = await navigator.mediaDevices.getDisplayMedia({
    audio: false,
    video: true,
  });
  // 接続する
...
document.addEventListener("DOMContentLoaded", (event) => {
  ...
  document
    .querySelector("#connectSendrecv")
    ?.addEventListener("click", connectSendrecv);
  document
    .querySelector("#connectScreenCapture")
    ?.addEventListener("click", connectScreenCapture);
  document
    .querySelector("#disconnectSendrecv")
    ?.addEventListener("click", disconnectSendrecv);
  document
